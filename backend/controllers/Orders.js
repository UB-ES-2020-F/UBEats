...
async function update(req, res)
{
    ...
    if(order.error)
        return res.status(403).send({"message": "Could not update order", error: order.error})
...
async function updateItems(req, res)
{
    ...
    if(!(params.order_id) || !(params.item_id))
        return res.status(403).send({"message": "Order ID / Item ID not specified"})
    ...
    if(orderItems.error)
        return res.status(403).send({"message": "Could not update orderItems", error: orderItems.error})
