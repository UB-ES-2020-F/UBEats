...
const SearchForm = () => {
        ...
        const [recommendedRest, setRecommendedRest] = useState([{name:'No results found, try with a different name.'}]);

        const fetchRecommendations = async () => {
                console.log({'recommended value':inputValue})
                const items = await RestService.getRecommendedRestaurants(inputValue);
                setRecommendedRest(items);

                /*if (items.length > 0) {
                        setRecommendedRest(items);
                }else {
                        setRecommendedRest({name:'No results found, try with a different name.'})
                }*/
